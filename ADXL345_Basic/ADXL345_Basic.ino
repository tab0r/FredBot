...
int x,y,z;
//These varialbes will hold converted accelerometer values
float xg,yg,zg;
...
void loop(){
  ...
  Serial.print(z, DEC);
  Serial.print(',');
  Serial.println(abs(x)+abs(y)+abs(z), DEC);
  */
  ...
  Serial.print(zg, DEC);
  Serial.print(',');
  Serial.println(abs(xg)+abs(yg)+abs(zg), DEC);      
  delay(1000); 
}
...
void readRegister(char registerAddress, int numBytes, unsigned char * values){
  //Since we're performing a read operation, the most significant bit of 
